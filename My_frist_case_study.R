...
#Find top 10  the place where bike are being most picked up and retunred.
head(count(all_trips_v2,start_station_name, member_casual, rideable_type, sort = TRUE), n = 10 )
head(count(all_trips_v2,end_station_name, member_casual, rideable_type, sort = TRUE), n = 10 )

#Find top 10 when bike are most used
head(count(all_trips_v2,day_of_week, rideable_type, sort = TRUE), n = 10 )
head(count(all_trips_v2,day_of_week, member_casual, rideable_type,ride_length, sort = TRUE), n = 10 )
head(count(all_trips_v2,member_casual, rideable_type, sort = TRUE), n = 10 )
head(count(all_trips_v2,day_of_week,ride_length,rideable_type, sort = TRUE), n = 10 )
#Find top 10 where and when bike are most used.
head(count(all_trips_v2,day_of_week, start_station_name, sort = TRUE), n = 10 )


#Find more information about the blank station
count(filter(all_trips_v2,start_station_name == ''), start_station_name,rideable_type, member_casual, sort = TRUE)
count(filter(all_trips_v2,end_station_name == ''), end_station_name,rideable_type, member_casual, sort = TRUE)
...
all_trips_v2$month <- ordered(all_trips_v2$month, levels=c("July", "August", "September", "October", "November", "December", "January", "February", "March", "April","May", "June"))
...
#Analyze ridership data by type, bike type and weekday
all_trips_v2 %>% 
  mutate(weekday = wday(started_at, label = TRUE)) %>% 
  ...
  summarise(number_of_rides = n()
            ,average_duration = mean(ride_length)) %>% 
  arrange(member_casual, weekday)  %>% 
  ggplot(aes(x = weekday, y = average_duration, fill = rideable_type, color=member_casual)) +
  geom_col(position = "dodge")+
...
#Analyze between number of ride and month
all_trips_v2 %>% 
  ...
  ggplot(aes(x = month, y = number_of_rides, color=member_casual, fill = rideable_type)) +
  geom_col(position = "dodge") +
  facet_wrap(~rideable_type)+
  facet_wrap(~member_casual)

#Analyze between membership, day and bike type
all_trips_v2 %>% 
  mutate(weekday = wday(started_at, label = TRUE)) %>% 
  group_by(member_casual, weekday) %>% 
  summarise(number_of_rides = n()
